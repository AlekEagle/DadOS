kernel_source_files := $(shell find kernel/kernel -name *.cpp)
kernel_object_files := $(patsubst kernel/kernel/%.cpp, build/kernel/%.o, $(kernel_source_files))

x86_64_cpp_source_files := $(shell find kernel/x86_64 -name *.cpp)
x86_64_cpp_object_files := $(patsubst kernel/x86_64/%.cpp, build/x86_64/%.o, $(x86_64_cpp_source_files))
...
$(kernel_object_files): build/kernel/%.o : kernel/kernel/%.cpp
	mkdir -p $(dir $@) && \
	x86_64-elf-g++ -c -I kernel/include -ffreestanding $(patsubst build/kernel/%.o, kernel/kernel/%.cpp, $@) -o $@

$(x86_64_cpp_object_files): build/x86_64/%.o : kernel/x86_64/%.cpp
	mkdir -p $(dir $@) && \
	x86_64-elf-g++ -c -I kernel/include -ffreestanding $(patsubst build/x86_64/%.o, kernel/x86_64/%.cpp, $@) -o $@

$(x86_64_asm_object_files): build/x86_64/%.o : kernel/x86_64/%.asm
	mkdir -p $(dir $@) && \
	nasm -f elf64 $(patsubst build/x86_64/%.o, kernel/x86_64/%.asm, $@) -o $@
